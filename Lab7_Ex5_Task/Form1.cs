...
namespace Lab7_Ex5_Task
{
    public partial class Form1 : Form
    {
        ...
        {
            RichTextBox1.Invoke(PrintDlegateFunc, new object[] { ";\n" });
        }

        private int Summ(int Value)
        {

            RichTextBox1.Invoke(PrintDlegateFunc, new object[] { "Расчет для числа (" + Value + "): " });
            for (int trial = 2; trial <= Value; trial++)
            {
                ...
                if (isPrime)
                {
                    RichTextBox1.Invoke(PrintDlegateFunc, new object[] { " " + trial });
                }
            }
            return Value;
        }
